    ...
    static class Request
    {
        ...
        public static void RequestRealmVersion()
        {
            ...
            if (result != null)
            {
                ...
                string MaestryPath = ExecutionPath + ImageFileFolder + CurrentRealm.n.mastery.Replace('.', '_');
                if (!Directory.Exists(MaestryPath))
                {
                    Directory.CreateDirectory(MaestryPath);
                }
                string RunePath = ExecutionPath + ImageFileFolder + CurrentRealm.n.rune.Replace('.', '_');
                if (!Directory.Exists(RunePath))
                {
                    Directory.CreateDirectory(RunePath);
                }
                string ItemPath = ExecutionPath + ImageFileFolder + CurrentRealm.n.item.Replace('.', '_');
        ...
        public static MemoryStream RequestSummonerMaestryImage(int id)
        {
            string path = ExecutionPath + ImageFileFolder + MaestryImageFile + id;
            MemoryStream memoryStream = new MemoryStream();
            using (FileStream fileStream = File.OpenRead(path))
            {
                memoryStream.SetLength(fileStream.Length);
